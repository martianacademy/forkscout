...
| Category          | Change                                                                                                                                                                          |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Tool Output**   | Removed all manual truncations from `read_file`, `run_command`, `web_search`, `http_request` — full output now passed to the model                                              |
| **Web Tools**     | Removed `browse_web` and `browser_screenshot` built-ins — superseded by Playwright MCP (`browser_navigate`, `browser_screenshot`, `browser_click`, `browser_fill`, etc.)        |
| **MCP Servers**   | Added `github` (`@modelcontextprotocol/server-github`) and `playwright` (`@playwright/mcp`) as default MCP servers                                                              |
| **MCP Config**    | Environment variable substitution in MCP `env` config — values starting with `$` are resolved from `process.env` at connect time (e.g. `"$GITHUB_API_KEY"`)                     |
| **Sub-Agents**    | Removed tool output truncation from `extractFromSteps` — partial progress recovery now returns full tool results                                                                |
| **System Prompt** | Added BREVITY section to admin and sub-agent prompts — output tokens are ~5× more expensive than input; model now instructed to avoid filler, preamble, and redundant summaries |
    ...
    "agent": {
        ...
        "mcpServers": {
            ...
            "github": {
                ...
                "env": { "GITHUB_PERSONAL_ACCESS_TOKEN": "$GITHUB_API_KEY" }
            },
            "playwright": {
                "command": "npx",
...
| Tool          | Description                                                                       |
| ------------- | --------------------------------------------------------------------------------- |
| `run_command` | Execute shell commands (30s timeout, secret scrubbing, full output no truncation) |
| `web_search`  | Search the web via SearXNG (JSON API, fast); falls back to Chromium scraping      |
...
| `github`              | stdio     | List repos, read files, search code, issues, PRs — uses `GITHUB_API_KEY` from `.env` |
| `playwright`          | stdio     | Full browser automation — navigate, click, fill forms, screenshot, extract content   |
...
| Step limit per agent | 20          |
| Timeout per agent    | 600 seconds |
| Model tier           | balanced    |
| Retry attempts       | 5           |

**Allowed tools**: ALL parent tools — file read/write, web, commands, memory read/write, MCP tools, Playwright browser automation

**Blocked tools**: `spawn_agents` (no recursion), `safe_self_edit`, `self_rebuild`
