...
//   chain sessions   → .agents/chats/self/               (full history, shared across chain)
//   parallel workers → .agents/chats/self-{key}/         (isolated, no prior history passed)
//   aggregator       → .agents/chats/self-agg-{batch}/   (isolated)
    ...
    execute: async (input) => {
        ...
        if (!input.wait) {
            ...
            return {
                success: true,
                queued: true,
                note: "Next session triggered in background. History at: .agents/chats/self/",
            };
        }
    ...
    inputSchema: z.object({
        ...
        tasks: z.array(z.object({
            ...
            prompt: z.string().describe(
                ...
                "1) The actual task. " +
                "2) Where to write results: .agents/tasks/{batch_name}/{session_key}-result.md. " +
                "3) When finished, mark done by flipping '- [ ] `{session_key}`' to '- [x] `{session_key}`' " +
                "in .agents/tasks/{batch_name}/plan.md."
            ),
        ...
        aggregator_prompt: z.string().describe(
            ...
            "Should: read all result files from .agents/tasks/{batch_name}/, " +
            "compile a final summary, send it via telegram_message_tools to notify the user, " +
            "then delete .agents/tasks/{batch_name}/ to clean up."
        ),
        chat_id: z.number().optional().describe(
...
export const manage_workers = tool({
    description:
        ...
        "cancel: stops the monitor, deletes saved state — task files in .agents/tasks/{batch}/ are kept. " +
        "delete: full cleanup — stops monitor, deletes saved state AND the .agents/tasks/{batch}/ directory entirely.",
    inputSchema: z.object({
    ...
    execute: async (input) => {
        ...
        if (input.action === "cancel") {
            ...
            return {
                success: true,
                note: `Batch "${input.batch_name}" cancelled. Monitor state deleted. Task files in .agents/tasks/${input.batch_name}/ are kept.`,
            };
        }
...
export const list_active_workers = tool({
    description:
        "List all active task batches in .agents/tasks/. " +
    ...
    execute: async (_input) => {
        if (!existsSync(TASKS_DIR)) {
            return { success: true, batches: [], note: "No task batches found — .agents/tasks/ does not exist yet." };
        }
