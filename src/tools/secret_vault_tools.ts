...
export const secret_vault_tools = tool({
    description:
        "Manage named secrets in the encrypted vault. Secrets are NEVER shown to the LLM — only aliases are visible. " +
        "Actions: store (save value under an alias), list (show all aliases), delete (remove by alias), verify (check alias exists). " +
        "WHEN TO USE: saving API keys, passwords, tokens that will be used in tool calls; " +
        "checking if a secret is already stored before asking the user; " +
        "removing an outdated or rotated credential. " +
        "After storing, use {{secret:alias}} as a placeholder in ANY tool input (headers, body, command) — values are injected at runtime. " +
        "NEVER pass the actual secret value to another tool directly — always use the placeholder. " +
        "Example store: {action: 'store', alias: 'openai_key', value: 'sk-...'} → use as: {{secret:openai_key}} in http_request_tools headers. " +
        "Example list: {action: 'list'} → returns all known aliases without values.",
