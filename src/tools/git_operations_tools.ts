...
export const git_operations_tools = tool({
  description:
    "Run git commands for version control: status, commit, push, pull, log, branch, diff, stash, fetch. " +
    "WHEN TO USE: checking what changed (status, diff); committing code after a task (commit + push); " +
    "reviewing recent history (log); creating/deleting branches; stashing/restoring in-progress work. " +
    "WHEN NOT TO USE: arbitrary shell operations — use run_shell_command_tools; " +
    "typecheck before commit — use get_errors first. " +
    "Pass 'args' for extra flags (e.g. args: ['--oneline', '-10'] for short log). " +
    "Examples: commit: {command: 'commit', message: 'feat: add cron job support'} | " +
    "push: {command: 'push', remote: 'origin', branch: 'main'} | " +
