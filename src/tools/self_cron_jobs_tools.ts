...
// Manage cron jobs in .forkscout/self-jobs.json — list, add, remove, run now.
...
function writeJobs(jobs: SelfJobConfig[]): void {
    mkdirSync(resolve(process.cwd(), ".forkscout"), { recursive: true });
    writeFileSync(JOBS_FILE, JSON.stringify(jobs, null, 4), "utf-8");
}

export const self_cron_jobs_tools = tool({
    description:
        "Manage scheduled self-cron jobs stored in .forkscout/self-jobs.json. " +
        ...
        "Jobs fire on a cron schedule and the agent runs the given message as a task, then optionally sends the result to Telegram chat IDs.",
    ...
    execute: async (input) => {
        ...
        if (input.action === "add") {
            ...
            jobs.push(newJob);
            writeJobs(jobs);
            logger.info(`Job added: ${input.name} → ${input.schedule}`);
            return {
                success: true,
                message: `Job "${input.name}" added. It will run on schedule: ${input.schedule}. Restart the agent to activate it.`,
                job: newJob,
            };
        }
        ...
        if (input.action === "remove") {
            ...
            return { success: true, message: `Job "${input.name}" removed.` };
        }
