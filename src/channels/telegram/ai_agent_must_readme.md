...
| `access-requests.ts` | Persists access requests in `.agents/access-requests.json` and `.agents/auth.json`                                         |
...
| `addToAuthAllowList(userId, role)`                        | fn        | Adds userId to `.agents/auth.json`                                                           |

---

## History Storage

- Session key: `telegram-{chatId}` → `.agents/chats/telegram-{chatId}.json`
